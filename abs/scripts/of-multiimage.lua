...
function M.open(l) ofSetDataPathRoot(ofCanvas(this, 1):getDir()) if type(l) == "string" then filename = l loaded = imageTable[index]:load(filename) else filename = l[1] M.buffer(l[2]);
for i = 0, nb -1 do;
if ofWindow.exists then ofDisableArbTex() imageTable[i]:clear();
loaded = imageTable[i]:load(ofFile(filename):getEnclosingDirectory() .. i..".png") end;
end end if loaded then print("loaded " .. filename.." ".."to " .. math.floor(nb -1)) end end;
function M.index(i) index = i % nb end;
...
imageTable[index]:bind();
outlet:outletBang(0);
imageTable[index]:unbind() end;
